...
#### Two-speed within the cascade — the "instant feel" requirement (load-bearing for 4.1–4.3)

**Cascaded must not mean slow.** The target is the near-zero-latency feel of Claude.ai / ChatGPT voice mode, where the live speaking model is fast and spontaneous even if a touch less deep than the heavy reasoning model. We hit it with a **two-speed cascade** — the brain stays frontier; only the *live-speech reasoning tier* is speed-tuned. Two named tiers:

- **Speaking Jarvis (fast tier)** — the live conversational interface. In voice mode the agent's reasoning LLM defaults to the gateway **fast slot** (Groq `llama-3.1-8b` today — or a realtime-tuned conversational model) for **sub-second first-token**. This is what the master hears answer *immediately*.
- **Frontier Jarvis (full brain)** — the heavy model (`llama-3.3-70b`+), the 8 tools, Mem0, `agent/safety.py`, the `interrupt()` approval gate. **UNCHANGED.** The fast tier is the live *interface to* this brain, not a replacement: same graph, same tools, same memory, same safety. **Only model speed is tuned for the live layer — the system stays frontier.**

This preserves the §B cascaded decision and the LangGraph brain in full. Three pieces make it work, and each is a **decision for the master to confirm:**

1. **Fast-tier model choice.** Default = the gateway **fast slot** (`llama-3.1-8b`, already wired, Groq-fast). Alternatives: a dedicated realtime-conversational model (e.g. a Groq/Cerebras low-latency model), or keep `llama-3.3-70b` if Groq's TTFT proves already sub-second in practice. *Recommend:* start on the fast slot via a `voice_mode` contextvar (mirrors the `stream_tokens` pattern shipped in 4.0) so the routing is a one-line flip, measure real TTFT, then decide.
2. **Escalation logic** (fast → frontier). Options: **(A) always-fast** — the 8b handles the whole turn incl. tools (simplest, lowest latency; risk: smaller model is weaker at tool-calling — and the open-weights tool-schema fragility, `project_open_weights_tool_schema_and_conversation_poisoning`, is worse on smaller models); **(B) fast-first, escalate-on-need** via a free heuristic (escalate when the selected toolset is non-empty / contains an APPROVE-tier tool, or a cheap complexity signal fires) — no extra round-trip; **(C) router-classify** a tier per turn with a tiny pre-call (adds one fast classify call ≈ 100–200 ms). *Recommend (B)* — fast by default, escalate to frontier when tools are in play or complexity fires; no added round-trip on the common case.
3. **Filler masking.** When a turn escalates (or any stage runs long), Jarvis speaks an **instant filler** ("One moment, Sir…" / a soft thinking tone) the moment the turn starts computing, so the master never hears dead air while the frontier tier works. *Recommend:* a small rotating set of butler fillers, spoken via the same TTS path, triggered when first-token hasn't arrived within a short budget (~400 ms) or on a detected escalation.

> **Barge-in is architected from 4.1, even though full duplex lands in 4.3.** Every voice turn must be **cancellable** (asyncio task cancel of the in-flight graph turn + the TTS stream), the mic stays open during playback, and AEC is on from the first voice-out build — so when the master speaks over Jarvis it stops *instantly*. Building these in from 4.1 avoids a rip-up at 4.3. (See 4.§D-2.)
...
**1. Latency budget.** Target **time-to-first-audio < ~1.5 s** after the master stops speaking (conversational feel) — and the *felt* latency should approach the near-instant feel of Claude.ai / ChatGPT voice mode. Rough budget: endpointing/VAD hangover ~200–300 ms → streaming STT final ~100–300 ms after endpoint → LLM time-to-first-token **~150–400 ms on the fast tier** (`llama-3.1-8b`; the §B two-speed cascade — vs ~300–800 ms on the frontier model) → buffer the **first sentence** → streaming TTS first-audio ~75–300 ms (ElevenLabs `flash` / Cartesia at the low end; Piper local real-time-factor at the higher end). Four levers, all required: **(i)** the **fast tier by default** (§B) for sub-second first-token; **(ii)** **filler masking** (§B) — speak "One moment, Sir…" the instant a turn starts so escalation/tool latency is never silent; **(iii)** **never wait for the full LLM response** — chunk on sentence boundaries and pipe each sentence into TTS while the next generates; **(iv)** a **low-first-audio TTS** so time-to-first-audio is bounded by the first sentence, not the full turn. Every sub-phase that adds a stage must show it streams, and the live layer must hold the instant feel even when the frontier tier is doing the thinking underneath.
...
**7. Web-app auth (can't borrow Telegram's identity).** Telegram identifies the master ambiently by `chat_id`; the web app has none. Options: (a) **single-user passkey** Credentials provider (`MASTER_PASSKEY`, Auth.js v5); (b) **Google OAuth** restricted to the master's email; (c) **WebAuthn/platform passkeys** (best UX+security, more build). **→ 4.0 shipped (a)** — but as a **BFF (backend-for-frontend) proxy, not a browser-held JWT** (the implementation decision, confirmed): the browser holds only an opaque Auth.js session cookie; Next.js route handlers (`frontend/src/app/api/**`) verify the session server-side (`await auth()`) and proxy to FastAPI with `X-API-Key` attached **server-side only** (`frontend/src/lib/backend.ts`, guarded by `server-only`). **Why BFF over the direct-JWT plan:** no secret in the browser, no CORS, and it sidesteps the Auth.js-v5-JWE-vs-HS256 mismatch that `backend/app/security/auth.py` flags (Auth.js v5's default session token is an encrypted JWE, not an HS256 JWT the backend's `_verify_jwt` can read). The backend's Bearer-JWT path stays available for any future *direct* client. Edge-safe split config (`auth.config.ts` / `auth.ts`) keeps Node-only crypto out of the proxy bundle. (b)/(c) remain hardening lifts. The session is **long-lived** (no constant re-auth for voice/wake-word), and the always-listening mic is **gated behind an authenticated session** so a closed/locked dashboard isn't hot-mic.
...
- **BFF transport (shipped 4.0).** The browser never talks to FastAPI directly — Next.js route handlers verify the Auth.js session and proxy to the backend with a server-side `X-API-Key` (§D-7). The 4.0 streaming endpoint pipes the backend SSE `ReadableStream` straight through; voice (4.1+) adds the audio/WS legs over the same BFF boundary.
...
##### 4.3b — Local STT (faster-whisper) *(BUILT 2026-06-16)*
*Build:* replace Web Speech command capture with backend **faster-whisper** reusing the mic→WS pattern — closes the cloud/Chrome dependency and the privacy gap, single-stream (no contention), VAD-owned endpointing (no idle-drop). **Deps discipline:** new backend dep → pin + re-freeze `constraints.txt` + ONE controlled rebuild (verify the web pins hold).
*Shipped:* a third WS mode **"capture"** — same stream, Silero VAD endpoints the utterance (`CaptureEndpointer`, onset pre-roll carried across the vad→capture switch so a barge-in's first word survives) and faster-whisper (`base.en`, int8, bounded by `WHISPER_TIMEOUT_S`, cpu-threads-capped) transcribes on end-of-speech → `{"event":"transcript"}`. Model lands in the `hf_cache` volume (free carry-over), warmed at startup in the background (mirror of the reranker fix). Frontend: `useVoiceLoop` capture phase → "capture" mode + awaits the transcript; **Web Speech (`useSpeechInput`) deleted**; `voiceMachine` reducer unchanged. *Verified:* Piper→whisper closed loop transcribes verbatim through the WS; **speech-end→transcript ≈ 1.2–1.35 s** (700 ms VAD hangover + ~500 ms `base.en` CPU transcribe); 12/12 headless tests; clean build; works browser-agnostically (Brave). *Deferred to optimization:* streaming **partial** transcripts (let the brain start before end-of-speech) — first cut transcribes on VAD-end.
...
- **A1 — Chat persistence + auto-save.** *(BUILT + HARDENED 2026-06-17)* The conversation survives reload + navigation to/from Approvals. The backend already persists every turn (LangGraph checkpointer, per `thread_id`); the gap was purely frontend. **Server-authoritative single thread:** the master is one continuous web conversation whose id is resolved server-side from the authenticated identity (`runner.canonical_thread_id` → `web:master`), NOT a client-held `localStorage`/uuid — so it's identical across reloads, browsers, devices, and a cleared cache. A read-only `GET /api/chat/history` (reusing `runner._serialize_message`) replays it on mount via the BFF; the mount-restore hydrates only into an empty message list (race guard, never clobbers a started turn). The barge-in cancellation-recovery was hardened to **never wipe the now-permanent thread** (dropped the `reset_thread` escalation — the orphan-removal + `repair_orphaned_tool_calls` already make a still-dirty `state.next` safe). *Follow-ups (land with B):* `/api/chat/history` returns the FULL thread (no pagination) — once the canonical thread grows it must return-recent / paginate, bounded by B's compaction; cross-channel thread unification (web ↔ Telegram, one thread) is a later seam.
...
*Decisive-cards system (A2, Option A confirmed 2026-06-17):* an APPROVE-tier action always yields a structured **decision card** in the chat (never a text "shall I send it?"). Four states, all **persisted across reload in conversation position** (rendered from the `PendingApproval` rows, which carry status + timestamps, interleaved with message history by the approval↔tool_call linkage): **pending** (Approve/Reject + "…or tell me what to change") · **approved ✓** · **rejected ✗** · **discarded** (superseded by an edit — shown greyed so history records what was proposed before the change). Resolution is **LLM-natural, not keyword-matched**: when a decision is pending and a message arrives, the LLM judges intent against the pending action — approve / reject / edit(+change) / unrelated — and generalizes across decision types (email now, others later). An *edit* discards the current card and re-drafts a new pending card (loop until approve/reject). Works by **text and voice** (voice gated to the live decision, never ambient). **(Future) Non-blocking pending decisions** — today a pending decision *pauses the graph* (blocking; an unrelated message gets a gentle nudge — Option A). A future enhancement lets the master ask/do other things while a decision quietly waits and resolve it whenever; that's a bigger re-architecture (the decision can no longer be a graph-blocking `interrupt()`). Deferred, revisit.
...
- **(B) is now LOAD-BEARING + time-sensitive (flagged 2026-06-17).** A1's server-anchor funnels all web text + voice into the one permanent `web:master`, and every turn sends the FULL history (no windowing) → unbounded tokens / latency / cost as it grows. **Rolling token-aware compaction** (older conversation → summary, recent verbatim) is what bounds the single thread; it must not slip far behind A2/A3. It also backstops the `/api/chat/history` pagination follow-up noted in 4.A.
...
>
> **20.5b DONE (2026-06-10) — Phase 2 CLOSES.** v + w green (real-stack); coverage baseline **44%** committed (`backend/tests/coverage_baseline.txt`, no gate). The failing Mem0 recall test was test RESIDUE crowding recall (cleaned; teardown made reliable — Mem0's get_all pages + delete() is flaky) — a test edge, NOT a storage bug. Findings: (a) **Turn 26.5** — memory_search recall-quality degraded by un-consolidated, partly-CONTRADICTORY preference bloat (consolidation is stubbed; possible memory-reranker lift). (b) **AGENT-CORE reliability gap, NOT 26.5** — Groq native-`<function>`-text-as-content isn't caught by FallbackChatLLM (only the error variant is); tracked in `project_open_weights_tool_schema_and_conversation_poisoning.md`, trigger = Phase-3 multi-tool/research work OR an observed native-text response. (c) test-isolation hardening follow-up (2 pre-existing full-suite failures: resume_dedup ordering + tool_selector registry pollution). See the Turn 20.5b entry in `jarvis-frontier-upgrade.md`.
...
**Agent-core hardening** — gateway cost-attribution **Option C** (hybrid helper closing the agent_node + embedding + Mem0-extraction bypass surfaces: `project_agent_llm_cost_attribution_gap`, `project_embedding_cost_attribution_gap`); FallbackChatLLM string-match fragility canary (`project_groq_error_message_string_match_dependency`); open-weights tool-schema fragility (`project_open_weights_tool_schema_and_conversation_poisoning`); per-tool execution timeout (`project_per_tool_execution_timeout_gap`); subgraph topology (`project_subgraph_topology_for_phase3_or_4`); N-provider fallback chain (`project_n_provider_fallback_deferral`); cross-loop async-state rebind watch (`project_async_state_rebind_pattern`); **Groq-outage fallback resilience** — the gpt-4o-mini fallback DOES carry chat + email when Groq is quota-drained (confirmed working), but two edge weaknesses to strengthen on a sustained outage: the soft-cost-cap degrade target is the *Groq* fast slot (a no-op during a Groq outage), and `agent_node` bypasses the daily cost cap so chat spends uncapped on OpenAI while gateway-capped paths (the consent judge) can halt at the hard cap (`project_groq_outage_fallback_resilience_future`; same `agent_node`-bypass root as the cost-attribution gap); **context-window sizing + compaction design-vs-need** — the 6000-token compaction threshold is small; before resizing, answer whether compaction is design/show or a genuine long-context need (latency / cost / TPM / model limits), which decides the window; the context meter was fixed to "compaction headroom" (recent verbatim / threshold) but should fold the rolling summary back IN against a larger budget when the window is resized — frontend-only, the backend already emits the pieces (`project_context_window_sizing_future`).
...
**Briefing / Proactive check-in (5.4)** — **deliver-signal re-verification (PARKED)**: the `deliver_briefing()` upgrade depends on the PRIMARY model `llama-3.3-70b` actually emitting the signal on a check-in — never verified on llama (Groq's daily quota was exhausted every attempt; forced fallback=primary → all rate-limited). Re-verify on quota reset: force `settings.FALLBACK_MODEL = PRIMARY_MODEL` so a rate-limit ERRORS rather than silently using gpt-4o-mini (don't trust fallback evidence), run first-of-day-greeting + re-greet-with-fresh several times, report the real signal rate; if it under-fires, a narrow deterministic check-in fallback OR routing check-in-likely turns to a stronger model — the OFFER floor guarantees no regression meanwhile (`project_briefing_offer_underfires`). **Unit B — interrupt-carry-forward / TTS-accurate HWM** (no note yet, lives here): advance the briefing watermark only AFTER the brief is actually delivered/spoken, so an interrupted brief carries forward instead of being silently consumed — the single seam to refine is `briefing_state.mark_briefed`; the advance-on-return residual is documented inline in `briefing_tool`.
