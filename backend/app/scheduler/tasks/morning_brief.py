...
from app.briefing import build_push_digest, digest_to_payload, record_morning_brief, render_push
...
async def _send():
    ...
    parts = []
    # The structured digest the HUD persists (same content as the Telegram text). On a
    # digest-build failure it degrades to an empty/error payload, mirroring the text.
    payload: dict = {"empty": True, "total": 0, "days": [], "error": False}
    ...
    try:
        ...
        payload = {"empty": True, "total": 0, "days": [], "error": True}
    ...
    # Persist for the HUD FIRST (best-effort, never raises) so the brief surfaces in the
    # dashboard even if Telegram delivery is down — mirrors send_system_alert's order.
    await record_morning_brief(payload)

    # Telegram delivery — unchanged + independent of the HUD persist.
    ch = channel_registry.get(PRIMARY_ALERT_CHANNEL)
    await ch.send_alert(f"☀️ *Good Morning!*\n\n{full_brief}")
    logger.info("morning_brief_sent", parts_count=len(parts), hud_total=payload.get("total", 0))
