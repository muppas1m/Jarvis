...
# ---------------------------------------------------------------------------
# Consent tier (consulted by the decision judge, not the classifier): a soft
# affirmation is consent ONLY for reversible sends; everything else needs an
# EXPLICIT command. The partition lives in safety.py; default is fail-safe.
# ---------------------------------------------------------------------------
@pytest.mark.parametrize(
    "tool_name",
    ["email_send", "email_reply", "whatsapp_send", "draft_email_reply"],
)
def test_consent_tier_soft_for_sends(tool_name: str) -> None:
    from app.agent.safety import consent_tier

    assert consent_tier(tool_name) == "soft"


@pytest.mark.parametrize(
    "tool_name",
    [
        # destructive / irreversible / money — soft affirmation is NOT enough
        "calendar_create", "calendar_update", "calendar_delete",
        "booking_reserve", "book_restaurant", "search_flights", "browser_form_submit",
        # unlisted / unknown defaults to EXPLICIT (fail-safe)
        "some_future_tool", "delete_account",
    ],
)
def test_consent_tier_explicit_for_destructive_and_unknown(tool_name: str) -> None:
    from app.agent.safety import consent_tier

    assert consent_tier(tool_name) == "explicit"


def test_consent_soft_set_is_approve_tier_plus_the_synthetic_draft_action() -> None:
    # Every REAL soft-consent tool must itself be APPROVE-tier (a soft "that works" can only
    # apply to something that was going to be asked about). draft_email_reply is the one
    # SYNTHETIC judge-only action (the complex-email heads-up) — not a registered tool, and
    # approving it only DRAFTS (reversible), so it's the sole allowed non-APPROVE-tier member.
    from app.agent.safety import SOFT_CONSENT_TOOLS

    approve_tier = {t for t, lvl in TOOL_SAFETY_MAP.items() if lvl == SafetyLevel.APPROVE}
    real_tools = SOFT_CONSENT_TOOLS - {"draft_email_reply"}
    assert real_tools <= approve_tier
    assert "draft_email_reply" not in TOOL_SAFETY_MAP  # confirm it IS synthetic (judge-only)
